...
auto savePk = [](const PubKey& pk, const std::string& path) {
    ...
    uint64_t t2; std::memcpy(&t2, &pk.prm.tuple2_fraction, 8);
    io::put64(o, t2);
...
int main() {
    ...
    std::cout << "enc a = " << a << " b = " << b << "\n";

    auto ct_a = enc_value(pk, sk, a);
    auto ct_b = enc_value(pk, sk, b);

    fs::create_directories(dir);
    saveCts({ct_a}, dir + "/a.ct");
    saveCts({ct_b}, dir + "/b.ct");
    ...
    auto fa = dec_value(pk2, sk2, cta);
    auto fb = dec_value(pk2, sk2, ctb);
    auto fsum = dec_value(pk2, sk2, combine_ciphers(pk2, cta, ctb));

    std::cout << "dec a = " << fa.lo << " b = " << fb.lo << " a + b = " << fsum.lo << "\n";

    assert(fa.lo == a && fb.lo == b && fsum.lo == a + b);
    assert(fa.hi == 0 && fb.hi == 0 && fsum.hi == 0);
